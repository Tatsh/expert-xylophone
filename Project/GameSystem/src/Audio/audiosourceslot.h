...
/**
 * The AVFoundation sound-effect source manager, reached through @c AudioManager's
 * @c seAVPlayer ivar.
 *
...
public:
    /**
     * One registered sound source: the bound source object, a companion object, and its loop
     * flag.
     */
    struct SourceRecord {
        ...
        /**
         * Initialises the record: stores a copy of @p source and its loop flag.
        ...
        /**
         * Clears the record, releasing its two object references.
         * @return Always 1.
    ...
    /**
     * Zeroes the manager's fields to the empty state (its construction-time initialiser).
     * @ghidraAddress 0x4a5d4
     */
    void InitAudioSourceSlot();
    /**
     * Builds the mixer voice pool, the call-name map, and the source table.
     * @param nVoiceCount The number of mixer voices to create.
     * @ghidraAddress 0x4a5e8
     */
    void InitializeSourceManager(int nVoiceCount);
    /**
     * Adds a source, returning its slot index.
    ...
    /**
     * Registers a source under a call name, returning whether it was added.
     * @param source The source object to register.
    ...
    /**
     * Removes the source registered under a call name.
    ...
    /**
     * Acquires a playback bus for the source at the given index, returning its handle.
    ...
    /**
     * Acquires a playback bus for the source under a call name, returning its handle.
     * @param callName The call name the source was registered under.
     * @param volume The mixer gain, forwarded untouched to the mixer.
     * @return The play handle for the acquired bus.
    ...
    /**
     * Starts playback of the bus identified by a play handle.
    ...
    /**
     * Stops the bus identified by a play handle, returning whether it was playing.
    ...
    /**
     * Pauses the bus identified by a play handle.
    ...
    /**
     * Returns the playback status of the bus identified by a play handle.
    ...
    /**
     * Stops the bus identified by a play handle and detaches its source.
    ...
    /**
     * Pauses every audio bus.
     * @ghidraAddress 0x4a670
     */
    void PauseAllBuses();
    /**
     * Resumes every audio bus.
     * @ghidraAddress 0x4a680
     */
    void ResumeAllBuses();
    /**
     * Sets the volume of every audio bus.

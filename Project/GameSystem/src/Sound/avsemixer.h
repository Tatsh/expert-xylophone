...
public:
    /**
     * Builds the voice pool: allocates the array and fills it with @p nVoiceCount voices.
    ...
    /**
     * Finds the index of the first free voice (one whose status is not playing).
    ...
    /**
     * Acquires a free voice, binds @p dwSource to it, prepares it, and returns a play
     * handle.
     *
    ...
    /**
     * Resolves a play handle back to its voice, validating the packed id.
    ...
    /**
     * Whether the voice at @p nIndex is free (its status is not playing).
    ...
    /**
     * Detaches @p source from every voice currently bound to it.
    ...
    /**
     * Starts playback on the voice resolved from @p dwHandle.
    ...
    /**
     * Stops the voice resolved from @p dwHandle.
    ...
    /**
     * Pauses the voice resolved from @p dwHandle.
    ...
    /**
     * Stops the voice resolved from @p dwHandle and unbinds its source.
    ...
    /**
     * The playback status of the voice resolved from @p dwHandle.
    ...
    /**
     * Sets the volume of the voice resolved from @p dwHandle.
    ...
    /**
     * Sets the volume of every voice in the pool.
     * @param nVolume The volume on a 0..127 scale.
    ...
    /**
     * Suspends the mixer, pausing @c m_nVoiceCount voices.
     * @ghidraAddress 0x47d5c
     */
    void SuspendAllBuses();
    /**
     * Resumes the mixer, un-pausing @c m_nVoiceCount voices.

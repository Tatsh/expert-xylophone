...
/**
 * A single decoded sound: the source file's sample rate and channel count, and the
...
public:
    /**
     * Constructs an empty sound buffer with all fields zeroed.
     * @ghidraAddress 0x4d350
     */
    caSource() = default;
    /**
     * Frees the decoded PCM data block.
     * @ghidraAddress 0x4d39c
     */
    ~caSource();

    /**
     * Loads a sound file at a filesystem path into this buffer.
    ...
    /**
     * Opens the sound file at @p url, reads its format, and decodes the whole PCM block.
    ...
    /**
     * Reads the source file's data format into @p pAsbd and derives the decoded PCM buffer
     *        size.
     *
    ...
    /**
     * Decodes the whole source file into the PCM data block sized by @c ReadAudioFormat.
     *
    ...
    /**
     * Copies @p nCount bytes out of the decoded PCM block as a ring buffer, wrapping at the
    ...
    /**
     * Frees the decoded PCM data block and clears the buffer pointer and size, leaving the
     *        object reusable.
     *
    ...
    /**
     * The number of channels in the decoded PCM.

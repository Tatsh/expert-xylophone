...
public:
    ...
    /** @brief Releases the per-sprite attribute arrays, the vertex scratch, and the GL buffers. */
...
protected:
    /**
     * @brief Force-disables every render capability the sprite pass does not want, then enables
    ...
    /** @brief Diagnostic only: dumps every sprite of this batch on the armed frame. */
    void DebugSnapshot();
    /**
     * @brief Binds the batch's texture (or disables texturing) and points the texture-unit's
     * coordinate array into the vertex scratch; shared by both draw paths.
     * @param pRenderer The GL render-state backend.
     */
    void BindPassTexture(neGLESRenderer *pRenderer);
    /**
     * @brief The slow path: one instanced draw per sprite, each carrying its own transform matrix
    ...
    /**
     * @brief The shared per-sprite matrix-emit loop for the slow and world-space paths: builds each
    ...
    /**
     * @brief The fast path: axis-aligned quads baked in world space and drawn in one indexed call.
     * @param pRenderer The GL render-state backend.
     */
    void RenderAxisAligned(neGLESRenderer *pRenderer);
    /**
     * @brief Builds sprite @p nSprite's translation*rotation*scale transform into @p pOutMatrix.
    ...
    /**
     * @brief Selects which of the binary's two vtables this object would have had.
     *
    ...
    /**
     * @brief An 8-byte member present in the binary's 0x160-byte object but read or written by no

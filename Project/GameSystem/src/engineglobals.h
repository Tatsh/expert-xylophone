/**
 * @file
 * Shared engine data tables, layout metrics, localised UI strings, and palette colours,
 * seeded at
...
#ifdef __OBJC__
/**
 * The per-difficulty minimum valid edited score (basic, medium, hard), used by the score
...
/**
 * The per-difficulty maximum valid edited score (basic, medium, hard).
 * @ghidraAddress 0x3de4a8
 */
extern NSArray *g_pScoreMaxThresholds;
#endif
/**
 * Seeds @c g_pScoreMinThresholds and @c g_pScoreMaxThresholds. Run once at startup.
 * @ghidraAddress 0x148a70
 */
void BuildGaugeThresholdArrays(void);
#ifdef __OBJC__
/**
 * The network API request-descriptor table, keyed by endpoint name (startup,
...
#endif
/**
 * Seeds @c g_pApiRequestTable. Run once at startup.
 * @ghidraAddress 0x3394c
 */
void InitializeApiRequestTable(void);
#ifdef __OBJC__
/**
 * The macron-to-vowel katakana lookup table (89 entries): each katakana maps to its
...
/**
 * The small-kana-to-large-kana lookup table (11 entries): ァィゥェォ→アイウエオ,
...
/**
 * The voiced-kana-to-voiceless-kana lookup table (25 entries): each voiced or semi-voiced
...
#endif
/**
 * Seeds the three katakana normalisation lookup tables (@c g_pLowerToUpperTable,
...
/**
 * The play-field full-height layout Y coordinate: the live field height minus one full
...
/**
 * The play-field near-lane slope: the ratio of the near note row's offset to the
...
/**
 * The negative of @c g_flPlayfieldNearLaneSlope, used for the bands below the field centre.
 * Seeded by the play-field layout pass.
 * @ghidraAddress 0x3ce960
 */
extern float g_flPlayfieldNearLaneSlopeNeg;
/**
 * The far-lane slope: the ratio of the far note row's offset to the field-centre row scale.
...
/**
 * The negative far-lane slope: the ratio of the far note row's offset to the field-centre
...
/**
 * The play-field field height, in 1024-scaled layout units (the base scale times 1024).
...
/**
 * The play-field field-centre split, in layout units, subtracted from the portrait gauge
 * positions. Seeded by the play-field layout pass.
 * @ghidraAddress 0x3ce934
 */
extern int g_nPlayfieldCentreSplit;
/** The half-height layout row (field height minus 0x200). @ghidraAddress 0x3ce938 */
extern int g_nPlayfieldHalfHeightY;
/** The near-HUD row at field height minus 0x16. @ghidraAddress 0x3ce93c */
extern int g_nPlayfieldRow16;
/** The near-HUD row at field height minus 0x2c. @ghidraAddress 0x3ce940 */
extern int g_nPlayfieldRow2c;
/** The near-HUD row at field height minus 0x36. @ghidraAddress 0x3ce944 */
extern int g_nPlayfieldRow36;
/** The near-HUD row at field height minus 0x6c. @ghidraAddress 0x3ce948 */
extern int g_nPlayfieldRow6c;
/** The field-centre row scale (centre split minus 0x36), the slope denominator.
 * @ghidraAddress 0x3ce94c */
extern float g_flPlayfieldRowScale;
/** The near note row's top offset constant (0x97). @ghidraAddress 0x3ce950 */
extern int g_nPlayfieldNearRowTop;
/** The near note row at field height minus 0x97. @ghidraAddress 0x3ce954 */
extern int g_nPlayfieldNearRowBottom;
/** The near note row at field height minus 0x12e. @ghidraAddress 0x3ce958 */
extern int g_nPlayfieldRow12e;
/** The far note row's top offset constant (0x15e). @ghidraAddress 0x3ce964 */
extern int g_nPlayfieldFarRowTop;
/** The far note row at field height minus 0x15e. @ghidraAddress 0x3ce968 */
extern int g_nPlayfieldFarRowBottom;
/** The mid note row's top offset constant (0xfa). @ghidraAddress 0x3ce974 */
extern int g_nPlayfieldMidRowTop;
/** The mid note row at field height minus 0xfa. @ghidraAddress 0x3ce978 */
extern int g_nPlayfieldMidRowBottom;
/** The mid-lane slope: the mid row offset over the row scale. @ghidraAddress 0x3ce97c */
extern float g_flPlayfieldMidLaneSlope;
/** The negative mid-lane slope. @ghidraAddress 0x3ce980 */
extern float g_flPlayfieldMidLaneSlopeNeg;
/** The gauge row's top offset (centre split minus 0x3e). @ghidraAddress 0x3ce984 */
extern int g_nPlayfieldGaugeRowTop;
/** The gauge row's bottom offset (field height minus the top). @ghidraAddress 0x3ce988 */
extern int g_nPlayfieldGaugeRowBottom;
/** The extra-lane slope: a fixed -62 offset over the row scale. @ghidraAddress 0x3ce98c */
extern float g_flPlayfieldExtraLaneSlope;
/** The negative extra-lane slope. @ghidraAddress 0x3ce990 */
extern float g_flPlayfieldExtraLaneSlopeNeg;
/** The result row at field height minus 0xe4. @ghidraAddress 0x3ce994 */
extern int g_nPlayfieldRowE4;
/** The result row at field height minus 0x192. @ghidraAddress 0x3ce998 */
extern int g_nPlayfieldRow192;
/**
 * Recomputes the whole play-field vertical layout table from a base scale factor.
 *
...
/**
 * The two gauge Y base positions used in the alternate (non-zero) gauge mode: the top band's
 * base and the bottom band's base. Seeded by the play-field layout pass.
 * @ghidraAddress 0x3ce99c
 */
extern int g_nGaugeAltTopBaseY;
/** The alternate-mode bottom-band gauge Y base. @ghidraAddress 0x3ce9a0 */
extern int g_nGaugeAltBottomBaseY;
/**
 * The two gauge Y base positions used in the default (mode-zero) gauge layout: the top
...
/** The default-mode bottom-band gauge Y base. @ghidraAddress 0x3ce9a8 */
extern int g_nGaugeBottomBaseY;
/**
 * One chart-decode entry: an obfuscated Blowfish key and its length in bytes.
 *
...
/**
 * The per-decode-type Blowfish key table shared with the chart loader, indexed by the decode
...
/**
 * The number of decode types @c kChartDecodeKeys carries.
 *
...
/**
 * A shared layout metric of 100 points used across the customize and store screens.
 * @ghidraAddress 0x2ec6f8
 */
extern const double g_dCustomizeLayoutMetric100;
/**
 * The wide (pad) slider and section row height metric.
 * @ghidraAddress 0x2ee950
 */
extern const double g_dSliderRowHeightWide;
/**
 * The slider/section row height for the current device, seeded once at startup by
...
#ifdef __OBJC__
/**
 * The cached opaque-white UI colour, seeded once at startup by @c
 * InitializeUiColorConstants.
 * @ghidraAddress 0x3df560
 */
extern UIColor *g_pCachedWhiteColor;
/**
 * The cached near-white (0.97) UI colour, seeded by @c InitializeUiColorConstants.
 * @ghidraAddress 0x3df568
 */
extern UIColor *g_pCachedOffWhiteColor;
/**
 * The cached blue accent UI colour (RGB 0.012, 0.478, 1.0), seeded by
...
#endif
/**
 * The mascot message-balloon maximum width on the pad layout.
 * @ghidraAddress 0x2ee930
 */
extern const double g_dMascotMessageMaxWidthPad;
/**
 * The mascot message-balloon maximum width on the phone layout.
 * @ghidraAddress 0x2ee938
 */
extern const double g_dMascotMessageMaxWidthPhone;
/**
 * The shared translucent-panel background white value.
 * @ghidraAddress 0x2ec6a0
 */
extern const double g_dTranslucentAlpha;
/**
 * The mascot slide-in/out move animation duration, in seconds.
 * @ghidraAddress 0x2ec6a8
 */
extern const double g_dMascotMoveAnimDuration;
/**
 * The mascot message-balloon fade animation duration, in seconds, reused as the shared
 * translucent-cover white value.
 * @ghidraAddress 0x2eedc0
 */
extern const double g_dMascotMessageAnimDuration;
/**
 * The default explosion customize-effect size scale.
 * @ghidraAddress 0x2ef17c
 */
extern const float g_flDefaultExplosionEffectSize;
/**
 * The shared minimum flash opacity, reused as the store BGM push/pop fade duration.
 * @ghidraAddress 0x2ec6b4
 */
extern const float g_flFlashMinOpacity;
/**
 * The audio-manager resume fade-in time, reused as the shared short UI fade duration.
 * @ghidraAddress 0x2ec718
 */
extern const double g_dAudioManagerResumeFadeInTime;
#ifdef __OBJC__
/**
 * The localised "Delete" action-button title (pad layout).
 * @ghidraAddress 0x3cfbb0
 */
extern NSString *g_pLocalizedDelete;
/**
 * The localised "Download" action-button title (pad layout).
 * @ghidraAddress 0x3cfbc8
 */
extern NSString *g_pLocalizedDownload;
/**
 * The localised action-button title format for a purchasable store item (one @c %@ price,
 * localised from the @c "BUY (%@)" catalogue key).
 * @ghidraAddress 0x3cfb78
 */
extern NSString *g_pLocalizedBuyFormat;
/**
 * The localised "Error" alert title.
 * @ghidraAddress 0x3cfbc8
 */
extern NSString *g_pLocalizedError;
/**
 * The localised "INSTALL" action-button title shown for a purchased but not-yet-downloaded
 * store item.
 * @ghidraAddress 0x3cfc00
 */
extern NSString *g_pLocalizedInstall;
/**
 * The localised "INSTALLED" action-button title shown once a store item is fully downloaded.
 * @ghidraAddress 0x3cfc08
 */
extern NSString *g_pLocalizedInstalled;
/**
 * The localised "INSTALLING" action-button title shown while a store item downloads.
 * @ghidraAddress 0x3cfc10
 */
extern NSString *g_pLocalizedInstalling;
/**
 * The localised "Purchased" disabled-button title shown on the pack detail purchase button
 * once the pack has been bought.
 * @ghidraAddress 0x3cfd10
 */
extern NSString *g_pLocalizedPurchased;
/**
 * The localised "OK" button title.
 * @ghidraAddress 0x3cfce0
 */
extern NSString *g_pLocalizedOK;
/**
 * The localised "Cancel" button title.
 * @ghidraAddress 0x3cfb80
 */
extern NSString *g_pLocalizedCancel;
/**
 * The download-progress modal-dialog message format string (one @c %@ tune name).
 * @ghidraAddress 0x3cfbd8
 */
extern NSString *g_pDownloadingMessageFormat;
/**
 * The delete-confirmation alert message format string (one @c %@ tune name).
 * @ghidraAddress 0x3cfcb8
 */
extern NSString *g_pDeleteConfirmFormat;
/**
 * The localised "failed to connect to the server" message.
 * @ghidraAddress 0x3cfcc0
 */
extern NSString *g_pLocalizedServerConnectFailed;
/**
 * The localised message shown when the server returns no usable data.
 * @ghidraAddress 0x3cfd60
 */
extern NSString *g_pLocalizedServerNoData;
/**
 * The localised "update required" message format shown when the extend-note catalogue
...
/**
 * The localised message shown when the shop-master version is older than the app.
 * @ghidraAddress 0x3cfdc8
 */
extern NSString *g_pLocalizedSearchVersionMismatch;
/** The localised "Close" button title. @ghidraAddress 0x3cfba0 */
extern NSString *g_pLocalizedClose;
/** The localised "All" filter title. @ghidraAddress 0x3cfb70 */
extern NSString *g_pLocalizedAll;
/** The localised "Add to playlist" action title. @ghidraAddress 0x3cfb68 */
extern NSString *g_pLocalizedAddToPlaylist;
/** The localised "Create playlist" action title. @ghidraAddress 0x3cfba8 */
extern NSString *g_pLocalizedCreatePlaylist;
/** The localised "No play songs" empty-state message. @ghidraAddress 0x3cfcd8 */
extern NSString *g_pLocalizedNoPlaySongs;
/** The localised "New" badge title. @ghidraAddress 0x3cfcc8 */
extern NSString *g_pLocalizedNew;
/** The localised "SPECIAL" level title. @ghidraAddress 0x3cfca0 */
extern NSString *g_pLocalizedSpecial;
/** The localised "Playlist" title. @ghidraAddress 0x3cfcf0 */
extern NSString *g_pLocalizedPlaylist;
/** The localised "PlaylistName" field label. @ghidraAddress 0x3cfcf8 */
extern NSString *g_pLocalizedPlaylistName;
/** The localised "Return" button title. @ghidraAddress 0x3cfd58 */
extern NSString *g_pLocalizedReturn;
/** The localised "Sort " menu title. @ghidraAddress 0x3cfd80 */
extern NSString *g_pLocalizedSort;
/** The localised "Level" title. @ghidraAddress 0x3cfc20 */
extern NSString *g_pLocalizedLevel;
/**
 * The localised per-level titles "Level1" through "Level15".
...
/**
 * The remaining localised UI strings, all seeded at startup by @c CacheLocalizedUIStrings.
...
#endif
/**
 * Seeds every localised UI string global above from the main bundle's localisation table.
 * Run once at startup as a module initialiser.
 * @ghidraAddress 0x10090
 */
void CacheLocalizedUIStrings(void);
/**
 * Seeds @c g_dSliderRowHeight from the device layout. Run once at startup.
 * @ghidraAddress 0x1c0a78
 */
void InitializeSliderHeightConstant(void);
/**
 * Seeds the three cached UI colours. Run once at startup.
 * @ghidraAddress 0x1d52a0
 */
void InitializeUiColorConstants(void);
#ifdef __OBJC__
/**
 * The dimming-cover overlay colour (black at half alpha), seeded by
...
/**
 * Seeds @c g_aGaugePartsScale. Run once at startup.
 * @ghidraAddress 0x83cf0
 */
void InitializeGaugeAngleTable(void);
/**
 * The difficulty-number image centre offset added over the button centre, per device: the
...
/**
 * The phone layout's difficulty-number image centre offset, added over the button centre.
 * @ghidraAddress 0x3dc6f0
 */
extern CGPoint g_difficultyNumberOffsetPhone;
/**
 * Seeds @c g_difficultyNumberOffsetPad and @c g_difficultyNumberOffsetPhone. Run once at
 * startup.
 * @ghidraAddress 0xc933c
 */
void InitializeCGAffineTransformGlobals(void);
/**
 * The Twitter share-image element draw positions, indexed: 0 title, 1 artist, 2 difficulty,
...
/**
 * Seeds @c g_aTwitterImageDrawPos. Run once at startup.
 * @ghidraAddress 0x88f24
 */
void InitializeParticleOffsetTable(void);
/**
 * The setting-screen layout table: a cache of the panel and per-theme button-column geometry
...
/**
 * Seeds @c g_aSettingLayout. Run once at startup.
 * @ghidraAddress 0xec450
 */
void InitializeSettingLayoutGlobals(void);
/**
 * The extend-note view's difficulty-number image centre offset, per device (the same {x, y}
...
/**
 * The phone layout's extend-note difficulty-number image centre offset.
 * @ghidraAddress 0x3dc2b0
 */
extern CGPoint g_extendNoteNumberOffsetPhone;
/**
 * Seeds @c g_extendNoteNumberOffsetPad and @c g_extendNoteNumberOffsetPhone. Run once at
 * startup.
...
#ifdef __OBJC__
/**
 * The shared UI palette colours, indexed by the customise/playlist theme code.
...
#endif
/** The 32-point shared layout metric. @ghidraAddress 0x2ee9b0 */
extern const double g_dLayoutMetricThirtyTwo;
/** The 60-point shared layout metric. @ghidraAddress 0x2ee948 */
extern const double g_dLayoutMetricSixty;
/** The 80-point shared layout metric. @ghidraAddress 0x2ec6c8 */
extern const double g_dLayoutMetricEighty;
/** The 82-point customize-row layout metric. @ghidraAddress 0x3011c8 */
extern const double g_dCustomizeLayoutMetric82;
/** The store detail copyright label width. @ghidraAddress 0x2eea30 */
extern const double g_dStoreDetailCopyrightLabelWidth;
/** The wide-layout popup base Y origin. @ghidraAddress 0x2eea38 */
extern const double g_dPopupBaseOriginYWide;
/** The terms screen's button row height. @ghidraAddress 0x2ec6e0 */
extern const double g_dTermButtonRowHeight;
/** The searchable-spot name image maximum width. @ghidraAddress 0x2fcfd8 */
extern const double g_dNameImageMaxWidth;
/** The web-view loading cover's grey white value. @ghidraAddress 0x2ec708 */
extern const double g_dRBWebViewGrayViewWhite;
/** The navigation bar tint white value. @ghidraAddress 0x2eef50 */
extern const double g_dRBNavBarTintWhite;
/**
 * The autoresizing mask that flexes every margin and both dimensions (0x3f).
 * @ghidraAddress 0x310450
 */
extern const unsigned int g_dwAutoresizingMaskFlexibleAll;
/**
 * The autoresizing mask the web-view and map activity indicators use, flexing only the four
 * margins (0x2d).
 * @ghidraAddress 0x310460
 */
extern const unsigned int g_dwRBWebViewIndicatorAutoresizingMask;
/**
 * The Limelight experience-package title colours, as RGB triples indexed by the package
...
/**
 * The tutorial-pastel message-bubble clip rectangles, indexed: 0 head, 1 body, 2 left tail,
...
/**
 * The tutorial-pastel child layout points, indexed: 0 head, 1 body, 2 left tail, 3 right
...
/**
 * Seeds @c g_aTutorialPastelClipRects and @c g_aTutorialPastelPositions. Run once at
 * startup.

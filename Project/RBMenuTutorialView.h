/**
 * @file
 * The first-run tutorial overlay shown over the menu.
 *
...
/**
 * The first-run tutorial overlay view.
 *
...
/**
 * Create the overlay with the given frame and size its message content view from the current
 *        iPad idiom.
...
/**
 * Build the overlay content: the dimming base view and its eight grey spotlight layers, the
...
/**
 * Fade the overlay in for the given tutorial type, then start that tutorial step on
...
/**
 * Fade the overlay out, then tear it down and detach it from @c RBMenuView on completion.
 * @ghidraAddress 0x139e04
 */
- (void)hideAnimation;

/**
 * Prepare the overlay for a device rotation.
 */
- (void)willRotate;

/**
 * Re-lay the overlay after a device rotation.
 */
- (void)didRotate;
...
/**
 * Run the tutorial step of the given type against @p rootView, animating its appearance.
 *
...
/**
 * Run the tutorial step of the given type: pick the control it points at, lay out the
...
/**
 * The menu hub that presents this overlay, held weakly.
...
/**
 * The dimming base image view that hosts the eight grey spotlight layers.
...
/**
 * Whether an overlay transition is currently running; taps and rotation are ignored while
 *        set.
...
/**
 * The opaque black view laid over everything during a rotation.
...
/**
 * The root view supplied by the current step, whose child control the step highlights, held
 *        weakly.
...
/**
 * The specific control the current step points at, held weakly.
...
/**
 * Whether the current step's spotlight lets touches through to the highlighted control.
...
/**
 * The spotlight rectangle, in this view's coordinates, of the currently highlighted control.
...
/**
 * The bouncing cursor image drawn over the highlighted control.
...
/**
 * The pulsing touch-marker image drawn over the highlighted control.
...
/**
 * The message content view that holds the message layer, pastel bubble, and window layer.
...
/**
 * The layer name tag whose sublayers keep their animations across a re-layout.
...
/**
 * The layer name tag whose sublayers are removed on a re-layout.
...
/**
 * The framed message-window layer that holds the message artwork.
...
/**
 * The message-text layer whose contents rectangle is swapped per step.
...
/**
 * The pastel speech-bubble layer drawn beside the spotlight.
...
/**
 * The full message artwork atlas the per-step clips are cut from.
...
/**
 * The clip rectangle of the message text within the artwork atlas. Declared by the binary
...
/**
 * The live tutorial step identifier this overlay is showing, mirrored from
 *        @c RBTutorialManager.
...
/**
 * The message content view's width, derived from the iPad idiom.
 *
...
/**
 * The message content view's height, derived from the iPad idiom.
 *

/**
 * @file
 * The extend-note catalogue list model.
 *
...
/**
 * The catalogue loaded but contained no extend notes.
 * @param list The reporting list model.
 */
- (void)extendNoteListDownloadNothing:(nullable RBStoreExtendNoteList *)list;
/**
 * The optional-products request resolved the pack queued for an external launch.
 */
...
/**
 * The delegate notified as the catalogue loads.
...
/**
 * The parsed extend-note records loaded so far.
...
/**
 * The ordered product identifiers currently loaded, as boxed integers.
...
/**
 * The in-flight catalogue-page downloader, when a page fetch is active.
...
/**
 * The server catalogue dictionary held while its StoreKit products are being resolved.
...
/**
 * The in-flight StoreKit products request, when one is active.
...
/**
 * Whether a catalogue fetch (page download or products request) is currently in flight.
...
/**
 * Returns the cached extend-note record for the given product identifier.
...
/**
 * Returns the cached extend-note record for the given product identifier, creating and
...
/**
 * Begins fetching the next catalogue page.
 * @return @c YES when a fetch was started, @c NO when one was already in flight.
 * @ghidraAddress 0xbf1a0
 */
- (BOOL)startFetching;
/**
 * Cancels any in-flight catalogue-page download or StoreKit products request.
 * @ghidraAddress 0xbf338
 */
- (void)cancelFetching;
/**
 * Requests the single-pack optional-products catalogue queued for an external launch.
 * @ghidraAddress 0xc07f4
 */
- (void)optionalProductsRequest;

/**
 * Merges a resolved StoreKit products response with the server catalogue dictionary,

/**
 * @file
 * Reconstructed interface for the KONAMI Applilink SDK's @c ApplilinkUdid device-identifier
 * helper.
 *
...
/**
 * Advertising-identifier, UDID-generation, and keychain-persistence helper for the Applilink
 * SDK.
 */
@interface ApplilinkUdid : NSObject

/**
 * The shared @c ApplilinkPasteBoard that mirrors the keychain-stored UDIDs.
 */
@property(strong, nonatomic) ApplilinkPasteBoard *pasteBoard;

/**
 * The lazily-created shared singleton.
...
/**
 * Write a freshly generated UDID into the first empty pasteboard slot.
...
/**
 * Write the given UDID into the first empty pasteboard slot.
...
/**
 * Replace the pasteboard's slot 0 UDID with the given value.
 * @param udid The UDID to store.
 * @return The stored record, or @c nil on failure.
...
/**
 * Read the pasteboard UDID for a service name and storage index.
...
/**
 * The first stored (potentially invalid) pasteboard UDID.
...
/**
 * The first stored (potentially invalid) old pasteboard UDID.
 * @param error Set to a localised error when no data is present.
...
/**
 * Delete the pasteboard UDID for a service name and storage index.
 * @param serviceName The pasteboard service name.
 * @param storageIndex The storage slot index.
...
/**
 * Delete every stored pasteboard UDID, current and old.
...
/**
 * The stored advertising reward UDID, refreshing it from the keychain.
...
/**
 * Generate, hash, and persist a fresh advertising reward UDID.
 * @param error Set to a localised error on failure.
...
/**
 * Delete the advertising reward UDID at a keychain index.
...
/**
 * Delete every advertising reward UDID keychain entry.
...
/**
 * Store the old UDID in the keychain under the old service name.
...
/**
 * Read the old UDID from the keychain.
 * @param error Set to a localised error on failure.
...
/**
 * Store the new UDID in the keychain and the SDK core, keyed by the stored ad storage index.
...
/**
 * Add or update a keychain generic-password item that stores a UDID under a service key.
...
/**
 * Read and validate a keychain UDID, updating its modification date on read.
...
/**
 * Look up a single keychain generic-password item by service key.
 * @param service The keychain service key.
...
/**
 * Delete a keychain generic-password item by service key.
 * @param service The keychain service key.
 * @param error Set to a localised error on failure.
...
/**
 * Validate that a keychain attribute dictionary holds a well-formed, live UDID record.
...
/**
 * The keychain account (storage index) recorded for a service key.
 * @param service The keychain service key.
...
/**
 * Record the account (storage index) for a service key in the keychain.
 * @param service The keychain service key.
...
/**
 * The device advertising identifier hashed with MD5, when tracking is available and enabled.
...
/**
 * Whether the user permits advertising tracking on this device.
...
/**
 * Whether the operating-system version exposes the advertising identifier framework.
...
/**
 * The lowercase hex MD5 digest of a string's UTF-8 bytes.
...
/**
 * Attach the resolved UDIDs to a request parameter dictionary, honouring the priority type.
...
/**
 * Attach the primary UDID to a request parameter dictionary.
 * @param udidParameters The mutable parameter dictionary to populate.
...
/**
 * Whether the ad, current, and old UDIDs are three distinct values.
...
/**
 * Whether the current UDID differs from the SDK pasteboard UDID.
...
/**
 * The keychain service name for advertising UDIDs, prefixed with the server environment.
...
/**
 * The keychain service name for old UDIDs, prefixed with the server environment.
...
/**
 * Copy the pasteboard-stored UDID for the recorded storage index into the keychain.
 * @ghidraAddress 0x22e7a4
 */
+ (void)setUdidKeychainFromPasteBoard;

/**
 * Whether the shared pasteboard reports the non-pasteboard UDID flag.
...
/**
 * The device advertising identifier UUID string, resolved by name at runtime.
...
/**
 * The keychain bundle-seed identifier, read from a keychain access group.
...
/**
 * Log the pasteboard and SDK-core UDID state when the debug environment default is set.

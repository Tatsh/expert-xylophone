/**
 * @file
 * In-app-purchase receipt signing helpers.
 *
 * Computes salted SHA-256 hex digests over the receipt-check JSON payload so the server can
 * authenticate the client's verification request.
 *
...
/**
 * Digest builders that sign the IAP receipt-verification request sent to the game server.
 */
@interface StoreUtil : NSObject

/**
 * Compute the salted SHA-256 check digest for a receipt-verification payload.
...
/**
 * Compute the nonce-salted SHA-256 check digest for a receipt-verification payload.
 * @param jsonString The receipt-check JSON payload to sign.
...
/**
 * Whether a string is a usable @c http or @c https URL.
 *
...
/**
 * Build a random nonce string of the given length.
...
/**
 * Build the receipt-verification JSON payload for the original protocol.
...
/**
 * Build the receipt-verification JSON payload for the V2 protocol.
 * @param receipt The Base64-encoded App Store receipt.
...
/**
 * The V3 receipt-verification endpoint URL.
...
/**
 * Map a numeric pack identifier to its App Store product identifier.
...
/**
 * Build the tune-info download URL for a purchased tune.
...
/**
 * Build the paged pack-list catalogue download URL for a genre.
...
/**
 * Build the pack-detail info download URL for a pack.
 *
...
/**
 * Build the extend-note-detail info download URL for an extend note.
 *
...
/**
 * Build the manage-screen sort-metadata list download URL.
...
/**
 * Map an App Store product identifier to its numeric extend-note identifier.
...
/**
 * Build the paged extend-note catalogue list download URL.
...
/**
 * Build the user-age and purchase-limit-type check URL.
 * @return The user-age check endpoint URL.
 */
+ (nullable NSURL *)userAgeURL;

/**
 * Extract the affiliate product parameters carried by an iTunes URL.
...
/**
 * Map an App Store product identifier to its numeric pack identifier.
 * @param productID The product identifier string.
...
/**
 * Format a StoreKit product's localised price string.
 *
...
/**
 * Build the campaign serial-code verification request JSON payload.
...
/**
 * Build the campaign item-info request JSON payload.
 * @param campaignID The campaign identifier.
 * @return The JSON payload string.
 */

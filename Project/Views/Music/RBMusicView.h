...
/**
 * The music-select detail view presented over the music grid.
 *
...
/**
 * Create the detail view for the given music and build its whole panel.
 *
...
/**
 * Build the entire detail panel: the jacket, the name images, the BPM, the score, rank,
...
/**
 * Build the per-difficulty BPM digit image column, laying the digits out from @p Point.
...
/**
 * Build the animated select-line overlay (iPad idiom layouts only).
...
/**
 * Refresh the score, achievement-rate, rank, and full-combo readouts for the current
...
/**
 * Set the rank badge image for the given rank, or clear it for @c -1.
...
/**
 * Show or hide the replay-ghost indicator for the given difficulty, dimming it per the
 * user's ghost style.
...
/**
 * Route the per-difficulty sub-view update to the difficulty-specific handler.
...
/**
 * Select the given setting page (colour, difficulty, or CPU), animating the button highlight
 * and sub-view fade.
...
/**
 * Highlight the selected setting button and dim the others.
...
/**
 * Update the pastel and double-play button availability and the decide-button enablement
...
/**
 * Enable or disable the difficulty sub-view's buttons.
...
/**
 * Enable or disable the setting scroll and show or hide its page control.
...
/**
 * Page-control action: scroll the setting view to the selected page.
...
/**
 * Decide button: seed the replay data and start the game, or the tutorial game when due.
 * @ghidraAddress 0xd4028
 */
- (void)SelectDecideButton;

/**
 * Double-play button: clear the pastel modes, select the double game type, and start.
 * @ghidraAddress 0xd3fac
 */
- (void)SelectDoublePlayButton;

/**
 * White-pastel button: select the white-pastel mode and start (unless animating).
 * @ghidraAddress 0xd4620
 */
- (void)SelectWhitePastelButton;

/**
 * Black-pastel button: select the black-pastel mode and start (unless animating).
 * @ghidraAddress 0xd4694
 */
- (void)SelectBlackPastelButton;

/**
 * History button: toggle the play-history overlay for the current music and difficulty.
 * @ghidraAddress 0xd44d0
 */
- (void)SelectHistory;

/**
 * iTunes button: open the song's iTunes page through the hosting view controller.
 * @ghidraAddress 0xd4f54
 */
- (void)SelectItunes;

/**
 * Seed the shared user settings and game system from the current selection, then start play.
 * @ghidraAddress 0xd4708
 */
- (void)playGame;

/**
 * Seed the tutorial defaults, load the tutorial song, and start the tutorial game.
...
/**
 * Animate the detail panel in (or, when @p showAnimation is @c NO, snap it in without a
...
/**
 * Animate the detail panel out: commit the selection to the shared settings, stop the BGM,
...
/**
 * Retry starting the select BGM on the next run-loop turn until it plays.
...
/**
 * Run the one-time setting-scroll hint animation.
 * @ghidraAddress 0xd5d4c
 */
- (void)firstInfoAnimation;

/**
 * Show the one-time setting-scroll hint the first time the detail view opens.
 * @ghidraAddress 0xd5f38
 */
- (void)firstInfoAnimationCheck;

/**
 * Advance the setting-scroll hint after each scroll step, or finish it.
 * @ghidraAddress 0xd61e0
 */
- (void)setFirstScrollAnimation;

/**
 * Continue the setting-scroll hint once a hint scroll step ends.
...
/**
 * Update the setting page control and title images as the setting scroll moves.
...
/**
 * Background-tap handler: dismiss the detail panel when the tap is outside the panel.
...
/**
 * Gate the background-tap gesture so it does not fire on the panel or on a control.
...
/**
 * The decide button the tutorial highlights.
...
/**
 * The double-play button the tutorial highlights, or @c nil when it has not been built.
...
/**
 * The difficulty button the tutorial highlights for the given difficulty.
...
/** The music this detail view describes. */
@property(strong, nonatomic, nullable) MusicData *musicData;
/** The extended music record, when the extended (level 4) difficulty is selected. */
@property(strong, nonatomic, nullable) MusicDataExtend *extMusicData;
/** The music menu that presents this detail view, held weakly. */
@property(weak, nonatomic, nullable) RBMenuView *musicMenuView;

/**
 * Whether this detail view was reached from the random-select button.
 *
...
/** The white-pastel play button (held without ownership, matching the binary). */
@property(assign, nonatomic, nullable) UIButton *whitePastelButton;
/** The black-pastel play button (held without ownership, matching the binary). */
@property(assign, nonatomic, nullable) UIButton *blackPastelButton;
/** The replay-ghost indicator image view. */

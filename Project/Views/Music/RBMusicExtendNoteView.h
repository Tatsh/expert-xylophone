/**
 * @file
 * The extend-note (追加ノート) badge shown for a tune in the music-select screen.
 *
 * When a tune has a
...
/**
 * The music-select extend-note badge: a difficulty button plus a comment caption panel.
 *
...
/**
 * Create the badge for an extend note and populate its button and caption.
 *
...
/**
 * Build the difficulty button and the translucent comment caption panel.
 *
...
/**
 * A flash-pulse hook mirroring the @c UIImageView(RB) selector; the binary leaves it empty.
...
/** The extend-note identifier the badge advertises. @ghidraAddress 0x3cf54, 0x3cf64 */
@property(nonatomic, assign) unsigned int extendNoteID;
/** The horizontal layout offset applied on the wide (pad) Colette layout.
 * @ghidraAddress 0x3cff0, 0x3d000 */
@property(nonatomic, assign) float layoutOffset;
/** The owning music-select view, held weakly as a back-reference.
 * @ghidraAddress 0x3cf74, 0x3cf94 */
@property(nonatomic, weak, nullable) RBMusicView *musicSelectedBase;
/** The tappable difficulty button carrying the special-chart frame and level glyph.

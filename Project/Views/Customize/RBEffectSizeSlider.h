/**
 * @file
 * The explosion (bounds-effect-size) slider control used on the customise screen (created
...
/**
 * An effect-size slider with a fixed-point numeric digit readout.
 */
@interface RBEffectSizeSlider : UISlider

/**
 * Create the slider, building its track sprite, grip sprite, and digit-image readout, and
...
/**
 * Set the current value, clamp it to the range, reposition the grip, and refresh the digit
 * readout.
...
/**
 * Map a touch point to a half-unit value, snap it, and update the slider.
...
/**
 * The number of whole-digit places notionally drawn in the readout.
 */
@property(assign, nonatomic) int digit;

/**
 * The clamped current value (a bounds-effect size in half-unit steps).
 */
@property(assign, nonatomic) float value;

/**
 * The track sprite the grip slides along.
 */
@property(strong, nonatomic, nullable) UIView *baseView;

/**
 * The draggable grip sprite.
 */
@property(strong, nonatomic, nullable) UIImageView *gripView;

/**
 * The digit glyph images (@c cus_nms_0 ... @c cus_nms_9 followed by @c cus_nms_dot).
 */
@property(strong, nonatomic, nullable) NSMutableArray<UIImage *> *numImages;

/**
 * The digit-place image views that make up the readout: the whole digit, the decimal point,
 * and the fractional digit.
 */
@property(strong, nonatomic, nullable) NSMutableArray<UIImageView *> *numImageViews;

/**
 * The rectangle within the track along which the grip travels.
 */
@property(assign, nonatomic) CGRect barRect;

/**
 * The lowest selectable value (0).
 */
@property(assign, nonatomic) int barMin;

/**
 * The highest selectable value (3).
 */
@property(assign, nonatomic) int barMax;

/**
 * The track pixels per half-unit value, used to convert between touch position and value.
 */

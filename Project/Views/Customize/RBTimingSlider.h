...
/**
 * A judge-timing (delay-frame) slider with a signed numeric digit readout.
 */
...
/**
 * Map a touch point to a value, snap it to a whole frame, and update the slider.
...
/**
 * The number of digit places drawn in the readout.
 */
@property(assign, nonatomic) int digit;

/**
 * The clamped current value (a whole delay-frame offset).
 */
...
/**
 * The digit glyph images (@c cus_nms_0 ... @c cus_nms_9 followed by @c cus_nms_minus).
 */
@property(strong, nonatomic, nullable) NSMutableArray<UIImage *> *numImages;

/**
 * The digit-place image views that make up the readout, most significant first.
 */
...
/**
 * The lowest selectable value (-10).
 */
@property(assign, nonatomic) int barMin;

/**
 * The highest selectable value (10).
 */
@property(assign, nonatomic) int barMax;

/**
 * The track pixels per unit value, used to convert between touch position and value.
 */
@property(assign, nonatomic) float step;

/**
 * A per-step value declared alongside @c step; the binary never messages its accessors.
 */

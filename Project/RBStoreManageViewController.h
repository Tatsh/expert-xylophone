/**
 * @file
 * The purchase-management store page.
 *
...
/**
 * The purchase-management store page, the manage tab's root controller.
 *
...
/**
 * Builds the purchased-tune table, the sort view controller and its navigation controller,
...
/**
 * Presents the sort selector: pushes the sort controller on the phone, or toggles the sort
...
/**
 * Dismisses the sort selector (popping the pushed controller or dismissing the popover) and
...
/**
 * Switches the list to the given sort order, re-titling the sort button. When the sort
...
/**
 * Toggles the sort button between its ascending and descending titles and reloads the
...
/**
 * Returns the tune dictionary at the given section and row for the current sort order.
...
/**
 * Rebuilds the sorted, sectioned tune list from the given purchased-tune dictionaries for
...
/**
 * Scrolls the table back to the top.
 * @param sender The bar button item that fired the action.
...
/**
 * Toggles a section's expanded state and reloads it.
...
/**
 * The per-cell action button handler: re-downloads a not-yet-present tune (showing the host
...
/**
 * Downloads the tune and every purchased extend note for the working section and row
...
/**
 * Cancels any in-flight downloads and hides the host tab's modal dialog.
...
/**
 * The sort-metadata dictionary (artist and title readings keyed by tune identifier).
...
/**
 * The popover presenting the sort selector on the pad.
...
/**
 * The navigation controller wrapping the sort selector.

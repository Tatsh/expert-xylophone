/**
 * @file
 * The phone campaign item detail page, pushed from the campaign list.
 *
...
/**
 * Rebinds the campaign item and refreshes the whole detail layout to reflect it.
...
/**
 * Marks the item's install as complete and re-styles the acquisition button accordingly.
 * @param flag @c YES once the item has finished downloading.
 * @ghidraAddress 0x64f8
 */
- (void)setDownloadFlag:(BOOL)flag;
/**
 * Reports whether a downloadable tune's archive is already present on disk.
...
/**
 * Reveals the item and detail views and begins loading the artwork if needed.
 * @ghidraAddress 0x6798
 */
- (void)showItemInfo;
/**
 * Reveals the item detail if an item is bound.
 * @ghidraAddress 0x6924
 */
- (void)loadInfo;
/**
 * Starts looping playback of the downloaded audio sample.
 * @ghidraAddress 0x6978
 */
- (void)sampleStart;
/**
 * Stops any playing audio sample and resets the sample view.
 * @ghidraAddress 0x6ad4
 */
- (void)sampleStop;
/**
 * Opens the item's external link, stopping any playing sample first (bar-button action).
 * @param sender The tapped control.
 * @ghidraAddress 0x6c08
 */
- (void)pushExternalLink:(nullable id)sender;
/**
 * Handles the acquisition button: info download, terms description, update prompt, or
...
/**
 * Resets the sample overlay to the stopped state.
 * @ghidraAddress 0x6fdc
 */
- (void)sampleViewStop;
/**
 * Shows the sample overlay in the downloading state.
 * @ghidraAddress 0x70b4
 */
- (void)sampleViewDownloading;
/**
 * Shows the sample overlay in the playing state.
 * @ghidraAddress 0x7198
 */
- (void)sampleViewPlaying;
/**
 * Toggles sample playback when the artwork is tapped: download, cancel, or stop.
 * @ghidraAddress 0x7274
 */
- (void)handleTapArtworkView;
/**
 * Stops the sample when background music finishes (notification handler).
 * @param notification The finished-BGM notification.
 * @ghidraAddress 0x74ec
 */
- (void)finishBgm:(nullable id)notification;
/**
 * Opens the item's external link (link-button action).
 * @param sender The tapped control.
 * @ghidraAddress 0x7508
 */
- (void)pushLink:(nullable id)sender;
/**
 * Begins downloading the acquisition info for the shown item.
 * @ghidraAddress 0x78b8
 */
- (void)itemInfoDownload;
/**
 * Refreshes the whole detail layout: resizes the name label, banner, description, divider,
 * and copyright, and grows the scrolling content to fit.
 * @ghidraAddress 0xa970
 */
- (void)updateLayout;
/**
 * Cancels and clears every in-flight banner-artwork downloader.

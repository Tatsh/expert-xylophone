...
/**
 * The theme-selection popup presented over the music menu, paging one full-page artwork per
 * unlocked theme.
 */
...
/**
 * Create the theme popup, select the theme popup type, build its content, and mark the
...
/**
 * Build the theme popup content: the paging scroll view, one full-page artwork per unlocked
...
/**
 * Re-run the theme selection from the scroll view's current offset after a layout pass.
...
/**
 * OK-button handler: play the decide sound, disable the button, fade the popup out, then
...
/**
 * Snap the paged offset to the nearest theme page, update the selected theme, and enable
...
/** The paging scroll view that holds the per-theme artwork pages. */
@property(strong, nonatomic, nullable) UIScrollView *scrollView;
/** The Classic theme's full-page artwork. */
@property(strong, nonatomic, nullable) UIImageView *classicView;
/** The Limelight theme's full-page artwork. */
@property(strong, nonatomic, nullable) UIImageView *limelightView;
/** The Colette theme's full-page artwork. */
@property(strong, nonatomic, nullable) UIImageView *coletteView;
/** The OK button that commits the selected theme. */
@property(strong, nonatomic, nullable) UIButton *okButton;
/** The currently selected theme (an @c RBUserSettingDataTheme value). */
@property(assign, nonatomic) int thema;
/** The number of unlocked themes, driving the scroll view's content width. */
@property(assign, nonatomic) int unlockedThemaCount;
/** The owning settings view, held weakly. */

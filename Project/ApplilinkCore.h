...
/**
 * The Applilink SDK's core entry point.
 */
...
/**
 * Initialise the SDK core for an application and server environment.
 *
...
/**
 * Resume the SDK core, closing any open store and re-initialising from persisted state.
 * @ghidraAddress 0x214b00
 */
+ (void)resume;

/**
 * Regenerate the authentication session, invoking the block once the session is valid.
...
/**
 * Reset the SDK core's cached initialisation state.
...
/**
 * Set whether advert screens use the common navigation-bar appearance.
...
/**
 * Whether advert screens use the common navigation-bar appearance.
...
/**
 * Set whether the SDK localises using the device's preferred languages.
...
/**
 * Whether the SDK localises using the device's preferred languages.
...
/**
 * Set the tint colour of the SDK's loading indicator.
...
/**
 * The tint colour of the SDK's loading indicator.
...
/**
 * Flag the SDK as not currently used inside the store.
 * @ghidraAddress 0x214cb4
 */
+ (void)unusedInStore;

/**
 * Whether the SDK is currently used inside the store.
...
/**
 * Flag the SDK as built with the legacy pre-Xcode 6 toolchain.
 * @ghidraAddress 0x214cd8
 */
+ (void)buildUnderXcode6;

/**
 * Whether the SDK was built with the Xcode 6 (or later) toolchain.
...
/**
 * The SDK's main window, used as a fallback advert host.
...
/**
 * Whether the SDK's initialisation is currently in progress.
...
/**
 * Whether the SDK's initialisation-status flag is set.
...
/**
 * The Applilink application identifier stored in @c NSUserDefaults.
...
/**
 * The current UDID, preferring the advertising UDID when advertising tracking is
 * available.
...
/**
 * The cached UDID without recomputation.
...
/**
 * The cached advertising UDID without recomputation.
...
/**
 * The cached old UDID without recomputation.
...
/**
 * The UDID, computing and caching it from the keychain on first access.
...
/**
 * The pasteboard UDID, computing and caching it from the keychain on first access.
...
/**
 * The advertising UDID, computing and caching it on first access.
...
/**
 * The old UDID, computing and caching it from the keychain on first access.
...
/**
 * Whether either the UDID or the advertising UDID is available.
...
/**
 * Clear the stored UDID and, outside the local environment, its keychain records.
 * @ghidraAddress 0x2156c4
 */
+ (void)clearUDID;

/**
 * Store the advertising UDID in the SDK core.
...
/**
 * Clear the old-UDID keychain record and, when no UDID remains, the initialisation state.
 * @ghidraAddress 0x215864
 */
+ (void)clearKeyChainOldUDID;

/**
 * Clear the advertising-UDID keychain records outside the local environment.
 * @ghidraAddress 0x2159bc
 */
+ (void)clearAdUDID;

/**
 * Persist the cached advertising UDID to the pasteboard when a re-login is pending.
...
/**
 * Present the App Store product page for an application, unless already used in the store.
...
/**
 * Close any open App Store product page.
...
/**
 * The Applilink SDK development version string.
...
/**
 * Report that the advert did start back to a delegate.
 * @param appParam The request parameters.
 * @param delegate The advert delegate to notify.
...
/**
 * Report that the advert did appear back to a delegate.
 * @param appParam The request parameters.
 * @param delegate The advert delegate to notify.
...
/**
 * Report that the advert did disappear back to a delegate.
 * @param appParam The request parameters.
 * @param delegate The advert delegate to notify.
...
/**
 * Report an open failure back to a delegate with the given error and request parameters.
...
/**
 * Report a load failure back to a delegate with the given error and request parameters.
 * @param error The localised failure error.
...
/**
 * Report a generic failure back to a delegate with the given error and request parameters.
 * @param error The localised failure error.
...
/**
 * Report a link failure back to a delegate with the given error and request parameters.
 * @param error The localised failure error.

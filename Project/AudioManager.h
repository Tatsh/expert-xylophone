/**
 * @file
 * The game's central audio manager singleton.
 *
...
/**
 * The central audio manager singleton, also the delegate of its background-music and voice
...
/**
 * The shared audio manager, created on first use.
...
/**
 * Schedule the deferred engine start on the current run loop.
 *
...
- (void)systemStart;
/**
 * Start the engine subsystems now rather than waiting for the next run-loop pass.
 * @ghidraAddress 0x3d4b4
 */
- (void)systemStartBlock;
/**
 * Tear down the engine audio context and release the background-music and voice players.
 * @ghidraAddress 0x3d4c4
 */
- (void)systemTerminate;
/**
 * Stop the engine audio graph, pause every bus, and suspend both the music and voice
 * players.
 * @ghidraAddress 0x40d6c
 */
- (void)systemSuspend;
/**
 * Restart the engine audio graph, resume every bus, and resume both the music and voice
 *        players.
 * @ghidraAddress 0x40e00
 */
- (void)systemResume;
/**
 * Whether the deferred engine start has run.
...
/**
 * Load background music from an in-memory data object, replacing any current music.
...
/**
 * Load background music by copying a raw byte buffer into an @c NSData first.
...
/**
 * Load background music from a raw byte buffer without copying it.
 * @param bytes The raw encoded-audio bytes.
 * @param length The buffer length in bytes.
 * @param loop Whether the music should loop indefinitely.
 * @return @c YES when the player was created successfully.
...
/**
 * Load background music from a raw byte buffer without copying it, optionally freeing the
...
- (BOOL)loadBgmDataWithBytesNoCopy:(void *)bytes
                      ...
                      freeWhenDone:(BOOL)freeWhenDone
                            isLoop:(BOOL)loop;
/**
 * Play the loaded background music, fading in over @p time seconds when it exceeds the
 *        no-fade threshold.
...
/**
 * Stop the background music, fading out over @p time seconds when it exceeds the no-fade
 *        threshold.
...
/**
 * Pause the background music, fading out over @p time seconds when it exceeds the no-fade
 *        threshold.
...
- (BOOL)isPlayingBgm;
/**
 * The background music's current playback position, in seconds.
 * @ghidraAddress 0x3fe30
 * @return The current time, or @c 0 when there is no player.
 */
- (double)bgmCurrentTime;
/**
 * The background music's device time, in seconds.
 * @ghidraAddress 0x3fed0
 * @return The device current time, or @c 0 when there is no player.
 */
- (double)bgmDeviceCurrentTime;
/**
 * Seek the background music to a given position.
 * @ghidraAddress 0x3ff70
 * @param bgmCurrentTime The target position, in seconds.
 */
- (void)setBgmCurrentTime:(double)bgmCurrentTime;
/**
 * Seek the background music back to the start.
 * @ghidraAddress 0x40660
 */
- (void)seekBgmToTop;
/**
 * Push the current background music onto the stack and pause it, leaving no active music.
 * @ghidraAddress 0x4048c
 */
- (void)pushBgm;
/**
 * Restore the background music previously saved by @c -pushBgm.
 * @ghidraAddress 0x405a4
 */
- (void)popBgm;
/**
 * Stop the background-music player and release it, leaving no loaded music.
...
/**
 * Load a voice line from an in-memory data object, replacing any current voice.
...
/**
 * Play the loaded voice line, resuming from the saved position after a pause.
...
- (BOOL)stopVoice;
/**
 * Pause the voice line, saving its position so @c -playVoice can resume it.
 * @ghidraAddress 0x40860
 * @return @c YES when there was a player to pause.
 */
- (BOOL)onPauseVoice;
/**
 * Whether the voice line is currently playing.
 * @ghidraAddress 0x40948
 * @return @c YES when the voice player is playing.
 */
- (BOOL)isPlayingVoice;
/**
 * Release the loaded voice line and its player.
...
/**
 * Load a sound effect from a file, registering it under an optional call name and group.
...
- (int)loadSe:(nullable NSString *)path
     ...
     callName:(nullable NSString *)callName
        group:(int)group;
/**
 * Release a loaded sound effect addressed by call name or resource identifier.
...
/**
 * Release every loaded sound effect from both subsystems and clear the look-up tables.
...
/**
 * Play a sound effect addressed by call name or resource identifier, at its group volume.
...
/**
 * Play a sound effect at an explicit volume.
...
/**
 * Play a sound effect on a specific mixer group.
...
/**
 * Pause every active sound-effect instance.
 * @ghidraAddress 0x3f090
 * @return Always @c YES.
 */
- (BOOL)onPauseSeAll;
/**
 * Resume every active sound-effect instance.
 * @ghidraAddress 0x3f110
 * @return Always @c YES.
 */
- (BOOL)offPauseSeAll;
/**
 * Stop every active sound-effect instance.
 * @ghidraAddress 0x3f190
 * @return Always @c YES.
 */
- (BOOL)stopSeAll;
/**
 * Stop the background music, the voice line, and every sound effect.
...
/**
 * Set the playback volume for a mixer group.

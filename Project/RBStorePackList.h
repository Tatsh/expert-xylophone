 ...
 * progress to its @c StorePackListDelegate. It also caches the parsed packs keyed by pack
...
/**
 * A copy of the store-country code cached from the most recently resolved StoreKit products,
...
/**
 * The parsed packs, keyed by insertion order and cached across pages.
...
/**
 * The most recent catalogue page, retained while the StoreKit product request runs.
 * @ghidraAddress 0x1f34f0 (getter)
 * @ghidraAddress 0x1f3500 (setter)
 */
@property(nonatomic, strong, nullable) NSDictionary *tempPackList;
/**
 * The in-flight StoreKit product request, or @c nil when idle.
...
/**
 * The genre whose page is currently being fetched.
...
/**
 * The delegate notified of load results.
...
/**
 * Whether the current genre's pack list can be extended with another page.
...
/**
 * The number of available genres.
...
/**
 * The display names of every genre, in order.
...
/**
 * Build the genre models from the catalogue's genre payload.
 *
...
/**
 * The genre model at the given index.
...
/**
 * Begin fetching (or extending) the genre at the given index.
...
/**
 * Begin fetching (or extending) the given genre's pack list.
...
/**
 * The cached pack for the given identifier, if any.
...
/**
 * Create and cache a pack for the given identifier when it is not already cached.
 * @param packID The pack identifier.
...
/**
 * Whether a catalogue-page or StoreKit fetch is currently in flight.
...
/**
 * Cancel any in-flight catalogue-page and StoreKit fetch.
 * @ghidraAddress 0x1f07fc
 */
- (void)cancelFetching;

/**
 * Request the queued deep-link ("open store") pack's StoreKit product.

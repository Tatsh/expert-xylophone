...
/**
 * The phone-layout pack detail controller.
 */
...
/**
 * The delegate that drives the detail's purchase and download actions.
 */
@property(nonatomic, weak, nullable) id<StorePackViewDelegate> delegate;

/**
 * The pack the detail displays.
 */
@property(nonatomic, strong, nullable) StorePackInfo *packInfo;

/**
 * The table header showing the pack artwork and purchase button.
 */
@property(nonatomic, strong, nullable) StoreDetailHeaderView *headerView;

/**
 * The table listing the pack's tunes.
 */
...
/**
 * The stretchable even-row tune-cell background image.
 */
@property(nonatomic, strong, nullable) UIImage *packBGImage0;

/**
 * The stretchable odd-row tune-cell background image.
 */
@property(nonatomic, strong, nullable) UIImage *packBGImage1;

/**
 * The per-tune artwork downloads, keyed by the tune's table index path.
 */
@property(nonatomic, strong, nullable) NSMutableDictionary *artworkDownloaders;

/**
 * The network-error alert shown when the pack-detail download fails.
 */
@property(nonatomic, strong, nullable) UIAlertView *packinfoDownloadAlertView;

/**
 * Whether the controller is being dismissed, used to route the network-error alert's cancel
 * action to a detail close.
 */
@property(nonatomic, assign) BOOL closingFlag;

/**
 * Load and display the pack's detail, fetching the full tune list if it is not yet present.
 * @ghidraAddress 0x1d88a4
 */
- (void)loadInfo;

/**
 * Populate the header and table from the loaded pack and begin its header-artwork download.
...
/**
 * Set the detail's purchase state.
...
/**
 * Cancel and clear every in-flight tune-artwork download.
 * @ghidraAddress 0x1dc1e4
 */
- (void)stopDownloadArtworks;

/**
 * Open the given iTunes URL through the root view controller.
...
/**
 * A tune sample finished downloading; begin playing it if it is still the selected row.

...
// Unused by the shipped app, which drives the children through setupView: only.
// @ghidraAddress 0x1003df460
...
// ARC synthesises the binary's -dealloc, so no override is written here.
...
- (void)startJumpAnimationWithDuration:(float)duration delay:(float)delay {
    ...
    if (delay <= kJumpDelayThreshold) {
        ...
        // The binary's animations block is empty, so the delay is a plain wait.
        // @ghidraAddress 0x1b1710

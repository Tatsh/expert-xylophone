/**
 * @file
 * The pad-layout pack detail panel shown over the store's dimming cover.
 *
...
/**
 * The delegate that drives the panel's purchase, re-download, terms, and close actions.
...
/**
 * The pack the panel currently displays.
...
/**
 * The container view that hosts every per-pack subview (artwork, labels, button, tune rows).
...
/**
 * The four tune rows, one @c StorePackMusicView per pack tune slot.
...
/**
 * The pack jacket artwork view, which downloads its own image from the pack's artwork URL.
...
/**
 * The pack copyright-notice text view.
...
/**
 * The purchase / install / re-download action button.
...
/**
 * The spinner shown over the loading label while the pack detail downloads.
...
/**
 * The centred loading label shown while the pack detail downloads.
...
/**
 * The downloader that fetches the pack's full detail (tune list) when it is not yet loaded.
...
/**
 * The downloader for the currently-playing tune sample.
...
/**
 * The artist-site button that opens the pack's artist URL in the browser.
...
/**
 * Load and display the current pack, fetching its full tune list if it is not yet present.
 * @ghidraAddress 0xf9330
 */
- (void)loadInfo;

/**
 * Populate every subview from the loaded pack and begin its artwork downloads.
 * @ghidraAddress 0xf8a94
 */
- (void)showPackInfo;

/**
 * Cancel any in-flight pack-detail download.
 * @ghidraAddress 0xf8154
 */
- (void)cancelLoading;

/**
 * Stop any sample playback and cancel the sample download.
 * @ghidraAddress 0xf8238
 */
- (void)stopSample;

/**
 * Clear the displayed pack and reset every subview to its empty state.
 * @ghidraAddress 0xf7ddc
 */
- (void)removePackInfo;

/**
 * Whether every tune in the pack is already downloaded.
...
/**
 * Recompute the purchase button label from the current ownership and download state.
 * @ghidraAddress 0xf859c
 */
- (void)selfCheckButtonText;

/**
 * Set the purchase button label to the purchasable state, showing the price.
 * @ghidraAddress 0xf8714
 */
- (void)setButtonTextBuy;

/**
 * Set the purchase button label to the installable state.
 * @ghidraAddress 0xf8884
 */
- (void)setButtonTextInstall;

/**
 * Set the purchase button label to the installing state.
 * @ghidraAddress 0xf8934
 */
- (void)setButtonTextInstalling;

/**
 * Set the purchase button label to the installed state.
 * @ghidraAddress 0xf89e4
 */
- (void)setButtonTextInstalled;

/**
 * The purchase button action: begin buying or re-downloading the pack through the delegate.
...
/**
 * A tune row's link button action: open its iTunes URL through the delegate.
...
/**
 * A tune row's sample button action: toggle sample playback for that tune.
...
/**
 * The artist-site button action: open the pack's artist URL in the browser.
 * @ghidraAddress 0xfa270
 */
- (void)selectWebButton;

/**
 * A tune sample finished downloading: load and play it, then stop every other row.
...
/**
 * The terms label tap action: ask the delegate to show the terms and precautions.
 * @ghidraAddress 0xfa45c
 */
- (void)showTerm;

/**
 * Switch to the sequence-extension store for the given extend-note product identifier.

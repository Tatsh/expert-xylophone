...
//
// The table holds 28 descriptors. Kinds 14 to 24 are the tutorial message strips, 357 points wide
// and 24, 52 or 80 tall, and they carry UV indices 11 to 24; the highest index any descriptor uses
// is 24, which is the last UV rectangle, and the bytes after each table stop being plausible
// (a zero-sized sprite, then negative UV origins), so both lengths are bounded from the data

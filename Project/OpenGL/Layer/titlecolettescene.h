...
@class SePlayer;
#else
/** The Objective-C sound-effect player, opaque to a pure C++ translation unit. */
...
/**
 * A title-part touch hit-box: the top-left corner and the extent, in layout coordinates.
 *
...
/**
 * The theme-2 (Colette) title-screen scene: the campaign parts-based title shown before the
 * music list.
 *
...
public:
    /** The number of cached title textures the scene builds. */
    static constexpr int kTextureCount = 4;
    /** The number of part sprite instancers the scene builds. */
    ...
    /**
     * Constructs the scene: chains the scene base, installs the title dispatch table, and
    ...
    /**
     * Destroys the scene: releases its cached textures, part sprites, and sound-effect
    ...
    /**
     * The per-frame task callback: dispatches on the scene state.
     *
...
private:
    /**
     * Releases the cached textures and part sprites and terminates the sound-effect player.
     *
    ...
    /**
     * State 0: loads the campaign textures and part sprites, starts the title BGM, loads the
    ...
    /**
     * State 1: starts the title BGM and advances to the main loop.
     * @ghidraAddress 0x57a64
     */
    void StartMusic();

    /**
     * State 2: scrolls and animates the title parts, handles touch, and drives transitions.
    ...
    /**
     * The main loop's touch pass: tracks one touch, hit-tests the menu boxes on a fresh
     * touch, and classifies a flick into the hidden-gesture state machine on release.
     */
    void ProcessTitleTouch();

    /**
     * Begins the corporate-logo exit: seeds the exit cross-fade, stops the BGM, plays the
    ...
    /**
     * State 3: once the audio has fully started, tears down the scene and opens the music
     * list.
    ...
    /**
     * Emits the title part sprites for the frame.
     * @ghidraAddress 0x5872c
     */
    void RenderSprites();

    /**
     * Emits the campaign-portrait sprite layer for the frame.
     * @ghidraAddress 0x59474
     */
    void RenderCampaignPortrait();

    /**
     * Emits one animated part sprite of a standard timeline window for the current clock.
     *
    ...
    /**
     * Emits one animated part sprite at an explicit position.
     *
    ...
    /**
     * Emits one title part's sprite into its instancer slot and records its hit-box rect.
     *
    ...
    // The sound-effect part in the landscape layout uses a nudged and grown rectangle; a part
    // without a hit-box is ignored.
    void RecordPartHitBox(unsigned int nPartId,
                          const S_VECTOR2 &drawPosition,
                          const TitlePartLayoutRecord &layout);

    /**
     * Advances the flick-gesture state machine, toggling the swing direction when the main
    ...
    /**
     * Rotates a swing-particle rest position around the logo pivot by the current swing
    ...
    /**
     * The Y counterpart of @c ComputeSwingParticleX.
     * @param flBaseX The particle's rest X.
     * @param flBaseY The particle's rest Y.
    ...
    /**
     * Advances the title cross-fade timer and updates the interpolated fade value.
     *
    ...
    bool m_bSeReady = {};     // +0x51: arms the sound-effect hit-box once its timer passes.
    // unsigned char m_aReserved52[2] = {}; // +0x52
    int m_nIdleTimer = {};     // +0x54
    int m_nReadyDelay = {};    // +0x58: seeded to 0x708.
    int m_nSeTimer = {};       // +0x5c
    int m_nSeAccumulator = {}; // +0x60: runs only while the sound-effect part is active.
    // unsigned char m_aReserved64[4] = {};             // +0x64
    ne::C_TEXTURE *m_apTextures[kTextureCount] = {}; // +0x68: bg, parts, parts_eff, and campaign.
    ne::C_SPRITE_INSTANCING_2D *m_apSprites[kSpriteSlotCount] = {}; // +0x88
    int m_aSpriteCount[kSpriteSlotCount] = {};                      // +0x3c8
    ...
    bool m_bExiting = {};          // +0x720: set during the corporate-logo exit.
    // unsigned char m_aReserved721[7] = {}; // +0x721
    float m_flGlowPhase = {};      // +0x728
    int m_nActiveTouchId = {};     // +0x72c: -1 when none.
    int m_nGestureState = {};      // +0x730
    bool m_bGestureTriggered = {}; // +0x734: latched when a flick sequence completes.
    bool m_bSwingToggle = {};      // +0x735: also read as the "actively swinging" flag.
    // unsigned char m_aReserved736[2] = {}; // +0x736
    int m_nSwingDelta = {};    // +0x738: +1 or -1.
    int m_nSwingPhase = {};    // +0x73c: in degrees.
    bool m_bHinabitaMode = {}; // +0x740: the hidden campaign toggle.
    bool m_bSeTriggered = {};  // +0x741
    // unsigned char m_aReserved742[2] = {}; // +0x742
    float m_flViewportWidth = {};                      // +0x744: cached from the game system.
    float m_flViewportHeight = {};                     // +0x748
    TitleHitRect m_aHitBox[kHitBoxCount] = {};         // +0x74c
    S_VECTOR2 m_aPartAnchor[kPartAnchorCount] = {};    // +0x7cc
    S_VECTOR2 m_aSwingParticle[kPartAnchorCount] = {}; // +0x82c
    // unsigned char m_aReserved88c[4] = {};              // +0x88c
    SePlayer *m_pSePlayer = {}; // +0x890
};

/**
 * The 104-record per-sprite part-layout table binding each part's texture and placement.
 *
...
/**
 * The part UV-rectangle tables, selected by a part's render type and the orientation.
 *
...
/**
 * The ring of twelve part anchor positions the title arranges its campaign parts around.
 *
...
/**
 * The default part UV-rectangle table, serving the background and any non-typed part.
 *

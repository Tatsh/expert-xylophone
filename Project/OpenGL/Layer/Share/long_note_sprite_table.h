...
/**
 * The exclusive upper bound of the first type range that takes its height from the layout
 * table (and scales both axes) rather than from the caller's length argument.
 */
constexpr int kLongNoteBodyBoundLow = 0x14;

/**
 * The inclusive start of the second type range that takes its height from the layout table
 * (and scales both axes) rather than from the caller's length argument.
 */
constexpr int kLongNoteBodyRangeStart = 0x18;

/**
 * The exclusive end of the second type range that takes its height from the layout table
 * (and scales both axes) rather than from the caller's length argument.
 */

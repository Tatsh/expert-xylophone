...
/**
 * The slide-note judge-result layer (the spinning result marks a slide note leaves behind).
 *
...
public:
    ...
    /** The sprite-graphic kinds @c CreateSprite can emit. */
    ...
    /**
     * The process-wide slide-note result layer, created on first use.
    ...
    /**
     * Lazily builds the result sprite batch: loads the gm_parts1 atlas, creates the batch
    ...
    /**
     * Queues a slide-note result mark at @p position for the current frame.
     *
...
private:
    /**
     * Constructs the layer, chaining the base constructor, clearing its header fields, and

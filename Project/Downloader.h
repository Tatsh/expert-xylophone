/**
 * @file
 * A thin block-and-delegate façade over @c RBHttpUtil.
 *
...
/**
 * A lifecycle callback block, passed the downloader that fired it.
 */
typedef void (^DownloaderBlock)(Downloader *downloader);

/**
 * Delegate callbacks delivered on the main thread when the matching block is unset.
 */
@protocol DownloaderDelegate <NSObject>
@optional
/**
 * Sent as the transfer makes incremental progress, when @c proceedBlock is unset.
...
/**
 * An asynchronous HTTP request wrapper with block or delegate completion.
 */
@interface Downloader : NSObject

/**
 * The underlying HTTP connection this façade drives and forwards to.
...
/**
 * Initialise a GET request, or a download-to-file request when @p filePath is non-nil.
...
/**
 * Initialise a POST request with a body and content type.
 * @param url The URL to request.
...
/**
 * Start the request, delivering callbacks to the given blocks.
...
/**
 * Detach the delegate and cancel the underlying connection.
...
/**
 * The received response body parsed as JSON.
...
/**
 * @c RBHttpUtil progress callback: fires @c proceedBlock, else forwards to the delegate.
 * @param downloader The downloader reporting the progress.
 * @ghidraAddress 0x73050
 */
- (void)downloaderProceed:(Downloader *)downloader;
/**
 * @c RBHttpUtil completion callback: fires @c successBlock, else forwards to the delegate.
 * @param downloader The finished downloader.
 * @ghidraAddress 0x731a8
 */
- (void)downloaderFinished:(Downloader *)downloader;
/**
 * @c RBHttpUtil failure callback: fires @c failureBlock, else forwards to the delegate.

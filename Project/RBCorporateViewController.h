/**
 * @file
 * The corporate/legal information view controller.
 *
 ...
 * base 0x100000000). Ghidra addresses are offsets relative to the image base. The class adopts
...
/**
 * The corporate/legal information view controller, presenting Konami's corporate web page in
...
/**
 * Create the controller and build the navigation-bar custom "back" left bar-button item
...
/**
 * Set the view background to white and build the centred, animating loading spinner.
 * @ghidraAddress 0xf0554
 */
- (void)viewDidLoad;

/**
 * Reveal the navigation bar for this controller.
...
/**
 * Lazily build the corporate web view (when absent), add it to the view, and load Konami's
 * corporate page.
 * @param animated Whether the appearance is animated.
...
/**
 * Tear the web view down (removing it from its superview) when leaving the screen.
...
/**
 * Handle the custom "back" bar-button tap: stop the loading spinner, hide the navigation
...
/**
 * Dismiss the controller without animation: stop the spinner, hide the navigation bar, and
 * pop the controller.
...
/**
 * Start the loading spinner before a navigation begins.
...
/**
 * Clear the shared URL cache as a page load starts.
...
/**
 * Finish a page load: clear the first-request flag, stop the spinner, and suppress the iOS
...
/**
 * Handle a page-load failure: stop the spinner and present the shared network-error alert
...
/**
 * Handle the network-error alert dismissal: pop the controller when this is the first
 * request.
...
/** Whether this is the first page request, gating the alert-driven dismiss. */
@property(assign, nonatomic) BOOL isFirstRequest;
/** The loading spinner shown during page loads. */
@property(assign, nonatomic, nullable) UIActivityIndicatorView *indicator;
/** The most recent request URL. */
@property(strong, nonatomic, nullable) NSURL *requestURL;
/** The web view presenting the corporate page. */

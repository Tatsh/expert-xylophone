/**
 * @file
 * The archivable unlock, campaign-point, and reward singleton.
 *
...
/**
 * The cosmetic item categories an unlock, add, or query can target.
 *
...
/**
 * An archivable singleton of the player's unlocked items, campaign points, and rewards.
 */
@interface RBExperienceData : NSObject <NSCoding>

/**
 * The reward application identifiers for a single companion application. This backing store
...
/**
 * The enciphered on-disk form of @c installedAppliIds, produced by @c encodeAppliIds:.
...
/**
 * Returns the shared experience-data singleton, unarchiving it from the user defaults or
...
/**
 * Seeds the default unlocked items, empty themes, and zeroed points on a fresh install.
 * @ghidraAddress 0x1bc104
 */
- (void)initialized;

/**
 * Reports whether a background-music item is unlocked.
...
/**
 * Reports whether a shot item is unlocked.
...
/**
 * Reports whether an explosion item is unlocked.
...
/**
 * Reports whether a frame item is unlocked.
...
/**
 * Reports whether a background item is unlocked.
...
/**
 * Reports whether a tune item is unlocked.
...
/**
 * Reports whether a theme item is unlocked.
...
/**
 * Reports whether an item of a given category is unlocked.
...
/**
 * Unlocks a background-music item.
 * @param type The background-music item identifier.
...
/**
 * Unlocks a theme item.
 * @param themaID The theme item identifier.
 * @ghidraAddress 0x1bad80
 */
- (void)addThemaID:(int)themaID;
/**
 * Unlocks an item of a given category.
 * @param type The item category, an @c RBExperienceItemType raw value.
 * @param ID The item identifier within that category.
...
/**
 * Records that a reward application has granted an application identifier, persisting the
 * change immediately.
...
/**
 * Returns the application identifiers granted by a reward application.
...
/**
 * Adds points to the campaign total of the currently active theme.
 * @param point The points to add.
 * @ghidraAddress 0x1bb21c
 */
- (void)addPoint:(float)point;
/**
 * Returns the campaign point total of the currently active theme.
 * @return The active theme's point total, or zero when no theme is active.
 * @ghidraAddress 0x1bb2fc
 */
- (float)getPoint;
/**
 * Resets the campaign point total for a theme.
...
/**
 * Rebuilds every unlocked-item set from the current level tables and saves.
 * @ghidraAddress 0x1bb3c4
 */
- (void)takeover;
/**
 * Recomputes the campaign point total from every cleared score record and saves.
...
/**
 * Reports whether the player has unlocked nothing beyond the default items.

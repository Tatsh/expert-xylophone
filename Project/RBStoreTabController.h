/**
 * @file
 * The in-app store tab controller.
 *
...
/**
 * The store's tab bar controller, hosting the pack, extend-note, manage, and campaign tabs.
 */
@interface RBStoreTabController : RBBaseTabBarController

/**
 * Builds the four store tabs and their navigation controllers, then selects the tab a
...
/**
 * Creates the dimming cover view and the modal download dialog, sizing the dialog and its
...
/**
 * Shows the modal download dialog with a fade-in, disabling its abort button and routing
...
/**
 * Hides the modal download dialog with a fade-out and stops the download UI on completion.
 * @return Always @c YES.
...
/**
 * Selects the store tab with the given index, unless a tab-switch animation is running.
...
/**
 * Re-opens the store for a queued open request, switching to the matching tab and forcing
...
/**
 * Back-button action: stops the pack promotion, restores the menu BGM, reloads the music

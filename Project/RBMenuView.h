...
/**
 * Create the menu hub owned by the given view controller and build all of its subviews.
...
/**
 * Build every subview of the hub: the paging background, the header and footer chrome, the
...
/**
 * Prepare for a device rotation: record the centred grid item, fade the grid and background
...
/**
 * Finish a device rotation: restore the equivalent page, fade the grid and background back
...
/**
 * Fade the whole menu in, load the music-select BGM and voices, reload the music data, and
...
/**
 * Fade the whole menu out, stopping the news and background effect, and run the closing
...
/**
 * Present @p view over the menu, removing the previously shown view first.
...
/**
 * Retry playing the music-select BGM on the next run-loop turn until it starts.
...
/**
 * Reload the menu's music data after the purchased-music catalogue, search, or playlist
...
/**
 * Rebuild @c musicList from the music manager, applying the active search text and the
...
/**
 * Present the detail view for the given music, animating it in.
...
/**
 * Pick and present a random music from the current list.
...
/**
 * Return a random integer in the inclusive range @p getRandamInt ... @p max, seeding the C
...
/**
 * Remove and release the selected-music detail view, then refresh the information badges.
...
/**
 * Handle the settings button: toggle the settings overlay.
 * @ghidraAddress 0xab9d4
 */
- (void)SelectSettingButton;

/**
 * Hide the settings overlay if it is showing.
 * @ghidraAddress 0xab9e0
 */
- (void)hideSettingView;

/**
 * Toggle the settings overlay: build and open it, or close it.
 * @ghidraAddress 0xaba74
 */
- (void)toggleSettingView;

/**
 * Handle the ranking button: dismiss the search bar and settings, then present the ranking
 * overlay.
...
/**
 * Handle the store button: dismiss the search bar and settings, then verify the terms
...
/**
 * Open the store tab controller, or re-open it when a pending store target is queued.
 * @ghidraAddress 0xad948
 */
- (void)StoreOpen;

/**
 * Detach and release the store tab controller, restoring the menu BGM, news, and background
 * effect.
...
/**
 * Terms-agreement completion callback that opens the store.
...
/**
 * Present the how-to-play overlay.
 * @ghidraAddress 0xabf94
 */
- (void)showHowToView;

/**
 * Present the customize overlay, starting the customize tutorial step if needed.
 * @ghidraAddress 0xac0bc
 */
- (void)showCustomizeView;

/**
 * Present the theme-selection overlay.
 * @ghidraAddress 0xac274
 */
- (void)showThema;

/**
 * Present the map-search screen: a pushed controller on phones, an overlay on pads.
 * @ghidraAddress 0xac348
 */
- (void)showSearchView;

/**
 * Present the staff-credits overlay.
 * @ghidraAddress 0xac564
 */
- (void)showCreditView;

/**
 * Present the notification-page screen: a pushed controller on phones, an overlay on pads.
 * @ghidraAddress 0xac638
 */
- (void)showNotificationPageView;

/**
 * Present the Applilink overlay.
 * @ghidraAddress 0xac808
 */
- (void)showApplilinkView;

/**
 * Present the terms-of-use screen: a pushed controller on phones, an overlay on pads.
 * @ghidraAddress 0xac8dc
 */
- (void)showTermView;

/**
 * Dismiss the currently shown overlay if it responds to @c hideAnimation.
...
/**
 * Start the animated background and mascot on the pastel theme.
 * @ghidraAddress 0xacaac
 */
- (void)startBGEffect;

/**
 * Stop the animated background and mascot on the pastel theme.
...
/**
 * Handle a tap on the news ticker: follow its link, or route to the store or notification
 * page.
...
/**
 * Start or refresh the news download, or advance the ticker when the cache is still fresh.
 * @ghidraAddress 0xaf0a0
 */
- (void)startNews;

/**
 * Restart the news download from the retry timer.
 * @ghidraAddress 0xaf2a8
 */
- (void)startNewsFromTimer;

/**
 * Advance the news ticker to the next non-empty news text and schedule the next change.
 * @ghidraAddress 0xaf350
 */
- (void)showNextNewsText;

/**
 * Stop the news ticker, cancel its timer, and cancel the news download.
 * @ghidraAddress 0xaf7e8
 */
- (void)stopNews;

/**
 * Server-date callback stub. The binary body is empty.
...
/**
 * Refresh the settings/store button flash badges from the unseen-content flags.
...
/**
 * News/terms download completion: parse the news JSON, update the store badge, seed the
...
/**
 * News download failure: schedule a retry timer.
...
/**
 * Rebuild the folded per-music search-term dictionary from the music manager.
 * @ghidraAddress 0xafa84
 */
- (void)createSearchDictionary;

/**
 * Slide the search bar in and start editing, seeding it with the backed-up query and, on
...
/**
 * Slide the search bar out, backing up the query and clearing the filter.
 * @ghidraAddress 0xb0eac
 */
- (void)hideSearchBar;

/**
 * Clear the search query and hide the search bar.
 * @ghidraAddress 0xb16dc
 */
- (void)tapSearchMusicCancel;

/**
 * Update the search-token array from the query, returning whether the token set changed.
...
/**
 * Split and fold a query string into its unique non-empty search tokens.
 * @param searchString The raw query string.
...
/**
 * Apply the current search tokens as the global search filter and reload the music data.
 * @ghidraAddress 0xb1d14
 */
- (void)exeSearchPickUp;

/**
 * Whether every current search token is found in the given music's search-term list.
...
/**
 * Build the tutorial overlay for the music-select or customize step when it is due.
 * @ghidraAddress 0xb52a0
 */
- (void)preStartTutorial;

/**
 * Start the pending tutorial step once its target control is on screen.
 * @ghidraAddress 0xb5678
 */
- (void)startTutorial;

/**
 * The tutorial song's cell the music-select tutorial step highlights.
...
/**
 * The song grid the music-select tutorial step highlights.
...
/**
 * The settings button the settings tutorial step highlights.
...
/**
 * The store button the store tutorial step highlights.
...
/**
 * Hide the mascot and search mascot for the duration of a tutorial step.
 * @ghidraAddress 0xb5c0c
 */
- (void)setPastelForTutorialStart;

/**
 * Restore the search mascot after a tutorial step (the mascot itself stays hidden).
 * @ghidraAddress 0xb5cb0
 */
- (void)setPastelForTutorialEnd;

/**
 * Tear down the tutorial overlay.
...
/**
 * Enter playlist add or delete mode, sliding the menu buttons to reveal the edit controls.
 * @ghidraAddress 0xb5ec4
 */
- (void)playlistEditStart;

/**
 * Leave playlist editing, sliding the menu buttons back and reloading the music data.
 * @ghidraAddress 0xb740c
 */
- (void)playlistEditFinish;

/**
 * Enable or disable the add/delete confirm button from the current edit selection.
 * @ghidraAddress 0xb8618
 */
- (void)playlistAddDelButtonUpdate;

/**
 * Handle the playlist add-confirm button: add the edit selection to the active playlist.
 * @ghidraAddress 0xb8754
 */
- (void)SelectPlaylistAddButton;

/**
 * Handle the playlist delete-confirm button: remove the edit selection from the active
 * playlist and reload.
...
/**
 * Handle the playlist finish button: persist the playlist and leave editing.
 * @ghidraAddress 0xb8aa4
 */
- (void)SelectPlaylistFinButton;

/**
 * Enter the given playlist edit mode when the current mode allows it.
...
/**
 * Present the page slider over the search bar area when nothing else is shown.
...
/**
 * Page-slider delegate callback: page the grid to the requested page and update the
...
/**
 * Toggle the music cells' visibility when the mascot is tapped.
...
/**
 * Slide the push-notification banner in when nothing blocks it.
 * @ghidraAddress 0xb4810
 */
- (void)showPushNotificationView;

/**
 * Push-notification action: route to the store, an external URL, or a web page.
 * @ghidraAddress 0xb4ae8
 */
- (void)actionFromPushNotificationView;

/**
 * Push-notification dismissal: restore or keep the search mascots hidden.
...
/**
 * Configure a music cell for the current playlist-edit state and load its artwork and
 * score.
...
/**
 * Advance the search-pick-up handler for a long-press on the grid.
...
/**
 * Handle the end of any grid scroll: snap the page, reconfigure visible cells, and page the
 * background.
...
/**
 * Collection-view will-layout callback. The binary body is empty.
...
/**
 * Collection-view did-layout callback: recompute the page count, snap the current page, and
...
/**
 * Collection-view touch-began forwarding. The binary body is empty.
...
/**
 * Collection-view touch-ended forwarding: dismiss the keyboard while the search bar shows.
...
/**
 * Debug no-op that walks the grid's subviews. The binary discards its results.
...
/** Whether the music cells are hidden (toggled by tapping the mascot). */
@property(assign, nonatomic) BOOL musicCellHidden;
/** Whether the next music selection is a battle-mode selection. */
@property(assign, nonatomic) BOOL battleMusicSelect;

/** The index of the news text currently shown in the ticker. */
@property(assign, nonatomic) int newsInfoIndex;
/** The grid item recorded before a rotation, used to restore the equivalent page. */
@property(assign, nonatomic) int prevIndex;
/** The current playlist edit mode. */
@property(assign, nonatomic) int playListEditMode;

/** The base Y position of the pastel search mascot, derived from the iPad idiom. */
@property(assign, nonatomic) float searchPastelPosBaseY;

/** The number of distinct paging background images. */
@property(assign, nonatomic) NSUInteger backgroundImageCount;
/** The paging background's current logical page. */
@property(assign, nonatomic) NSUInteger backgroundCurrentPage;

/** The current grid page index. Setting it updates the page label. */
@property(assign, nonatomic) NSInteger currentPageIndex;
/** The number of grid pages (never less than one). Setting it updates the page label. */
...
/** The search-cancel button beside the search bar. */
@property(strong, nonatomic, nullable) UIButton *searchCancelButton;
/** The backed-up search query, restored when the search bar is shown again. */
@property(strong, nonatomic, nullable) NSString *backUpString;
/** The current array of folded search tokens. */
@property(strong, nonatomic, nullable) NSMutableArray *searchArray;
/** The per-music folded search-term dictionary. */
...
/** The page slider popup, held weakly. */
@property(weak, nonatomic, nullable) RBMenuPageSliderView *pageSlider;
/** The set of music IDs selected during playlist editing. */

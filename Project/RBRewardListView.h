/**
 * @file
 * The reward list hosted by @c RBCustomView.
 *
...
/**
 * A view that hosts the @c RewardNetwork advert screen over the customize picker.
 *
...
/**
 * Create the reward list with the given frame and build its subviews.
...
/**
 * Build the web target view, the loading spinner, and the hidden close button.
 * @ghidraAddress 0x10d034
 */
- (void)setupView;

/**
 * Close the advert screen, then remove the spinner and the web target view from their
 * superview.
 *
...
/**
 * Reveal the close button and open the @c RewardNetwork advert screen inside the web target
...
/**
 * @c RewardNetwork delegate callback: the advert list has appeared. Stops the spinner and,
...
/**
 * @c RewardNetwork delegate callback: the advert list has disappeared. Closes the advert
...
/**
 * @c RewardNetwork delegate callback: the advert list failed to load. Stops the spinner,
...
/**
 * The rounded, clipped view that hosts the @c RewardNetwork advert screen.
...
/**
 * The large spinner shown while the advert list loads.
...
/**
 * The close button that dismisses the advert list, hidden until the list has appeared.
...
/**
 * The customize popup that owns and presents this reward list.
...
/**
 * Whether a hide transition is running.
...
/**
 * A convenience alias for @c parentCustomView that only forwards to its setter; the binary

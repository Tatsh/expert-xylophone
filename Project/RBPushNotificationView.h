/**
 * @file
 * The push-notification prompt overlay.
 *
...
/**
 * The push-notification prompt overlay slid down over the music-menu screen.
 *
...
/**
 * The background banner image view that holds the message label and is slid on- and
 * off-screen.
 */
@property(strong, nonatomic, nullable) UIImageView *bgView;

/**
 * The two-line label that shows the notification body text.
 */
@property(strong, nonatomic, nullable) UILabel *messageLabel;

/**
 * The notification body text, taken from the popped notification's @c body entry.
 */
@property(strong, nonatomic, nullable) NSString *message;

/**
 * The notification's target URL, taken from the popped notification's @c url entry.
 */
@property(strong, nonatomic, nullable) NSString *urlString;

/**
 * The one-shot timer that fires @c hideAnimationStart after the auto-hide delay.
 */
@property(strong, nonatomic, nullable) NSTimer *timer;

/**
 * The top inset, in points, applied above the banner (2.0 by default).
 */
@property(assign, nonatomic) float upMargin;

/**
 * The delegate messaged when the banner is tapped or dismissed.
 *
...
/**
 * Create the banner overlay with the given frame.
...
/**
 * Build the banner: the clear background, the notification background image view, the
...
/**
 * Load the next queued notification and slide the banner into view.
 * @ghidraAddress 0x18eac4
 */
- (void)showNotification;

/**
 * Pop the next queued notification from the app delegate, store its body and URL, and set
...
/**
 * Slide the banner down into place, play the show sound effect, and (on completion) start
 * the auto-hide timer.
...
/**
 * Stop the auto-hide timer and dispatch @c hideAnimation onto the main thread.
 * @ghidraAddress 0x18efa0
 */
- (void)hideAnimationStart;

/**
 * Slide the banner back off-screen above the top edge and, on completion, hide it and tell
...
/**
 * Tap handler: parse the banner URL through @c RBUrlSchemeManager, or hand an @c http URL
...
/**
 * Invalidate and clear the auto-hide timer.

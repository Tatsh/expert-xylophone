/**
 * @file
 * The background-music manager singleton.
 *
...
/**
 * The shared background-music manager, created on first use.
...
/**
 * Load the menu background track chosen by the user's stored preference, looping, replacing
 *        any current track.
 * @ghidraAddress 0x6a324
 */
- (void)LoadMusicSelect;
/**
 * Load a menu background track selected by its preference index, looping, replacing any
 *        current track.
...
/**
 * Load the title-screen background track for the user's current theme.
 * @ghidraAddress 0x6a3b4
 * @param loop Whether the track should loop indefinitely.
 */
- (void)LoadMusicTitleWithLoop:(BOOL)loop;
/**
 * Load the result-screen background track for the user's current theme.
 * @ghidraAddress 0x6a560
 * @param loop Whether the track should loop indefinitely.
 */
- (void)LoadMusicResultWithLoop:(BOOL)loop;
/**
 * Load a background track from an in-memory audio data object, marking a track as loaded
...
/**
 * Push any current track aside, then load a new background track from an in-memory audio
...
/**
 * Release the current track, popping and releasing a pushed-aside track as well, and clear
 *        both track flags.
...
/**
 * Play the loaded background track, fading in over @p time seconds.
...
/**
 * Pause the loaded background track, fading out over @p time seconds.
 * @ghidraAddress 0x6a03c
 * @param time The fade-out duration in seconds.
 */
- (void)PauseMusic:(float)time;
/**
 * Stop the loaded background track, fading out over @p time seconds.
 * @ghidraAddress 0x6a0c8
 * @param time The fade-out duration in seconds.
 */
- (void)StopMusic:(float)time;
/**
 * Seek the loaded background track back to its start, when a track is loaded.
...
/**
 * Push the current track onto the audio manager's background-music stack, marking it pushed
...
- (BOOL)pushMusic;
/**
 * Restore the track previously pushed aside by @c -pushMusic, marking it the active track
 *        again.
...
- (BOOL)popMusic;
/**
 * Whether a track has been pushed aside and not yet restored.
